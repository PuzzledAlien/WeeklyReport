...
namespace Linkup.Common
{
    ...
    public class JsonHelper
    {
        ...
        //替换Json的Date字符串    
        static readonly MatchEvaluator _convertMSJsonDateToDateStringEvaluator = new MatchEvaluator(ConvertMSJsonDateToDateString);
        //取出 \/Date(1422795002671+0800)\/
        //毫秒部分有可能为负数
        static readonly Regex _msJsonDateRegex = new Regex(@"\\/Date\(-*[0-9]*\+[0-9]*\)\\/");

        //将"yyyy-MM-dd HH:mm:ss"格式的字符串转为"\/Date(1294499956278+0800)\/"格式    
        static readonly MatchEvaluator _convertDateStringToMSJsonDateEvaluator = new MatchEvaluator(ConvertDateStringToMSJsonDate);
        static readonly Regex _dateStringRegex = new Regex(@"\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}");

        static readonly Regex _msJsonDateTotalMillisecondsRegex = new Regex(@"(?<=\\/Date\()-*[0-9]*(?=\+[0-9]*\)\\/)");
        ...
        {
            if (IsNullOrEmpty(json) || IsNullOrEmpty(propertyPath))
                return Empty;
        ...
        private static string ConvertDateStringToMSJsonDate(Match m)
        {
            ...
            result = $@"/Date({ts.TotalMilliseconds}+0800)/";
            return result;
        }
    }
}
