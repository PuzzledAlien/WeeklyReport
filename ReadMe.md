...
3. **升级到 .NET 10**
   ...
   - 将所有项目从 .NET Core 3.1 升级到 .NET 10
   - 使用 Dapper 替换不兼容的 Enterprise Library 数据库访问层
   - 简化 LogService 和 ExceptionHandlingService
   - 移除 BuildBundlerMinifier（ASP.NET Core 不兼容）

4. **多数据库支持**

   - 同时支持 SQL Server 和 MySQL
   - 在 appsettings.json 中通过 DatabaseProvider 配置切换
   - 添加 SQL 语法转换（SQL Server `[]` → MySQL `` ` ``）
   - 创建各数据库初始化脚本：
     - `Data/SQLServer/` - SQL Server 脚本
     - `Data/MySQL/` - MySQL 脚本
